...
# Get the index of the 'Unsafe' class
classes = rf_classifier.classes_  # List of class labels (e.g., ['Moderate', 'Safe', 'Unsafe'])
unsafe_index = list(classes).index('Unsafe')

# Extract probabilities for the `Unsafe` class
unsafe_proba = y_proba[:, unsafe_index]

# Combine probabilities with test data for analysis
test_data_with_proba = pd.DataFrame(X_test, columns=['lean_angle_deg', 'speed'])
test_data_with_proba['unsafe_proba'] = unsafe_proba

# Identify risky instances
risky_instances = test_data_with_proba[test_data_with_proba['unsafe_proba'] > 0.4]  # Threshold for high risk
print("Risky Instances (High Posterior Probability of Unsafe):")
print(risky_instances)
risky_instances_probabilities = test_data_with_proba['unsafe_proba']

# Compute the cumulative posterior probability
cumulative_posterior_probability = risky_instances_probabilities.sum() / len(test_data_with_proba)

print(f"Cumulative Posterior Probability of Risky Instances: {cumulative_posterior_probability:.4f}")
...
# Identify risky instances
risky_instances = test_data_with_proba[test_data_with_proba['unsafe_proba'] > 0.6]  # Threshold for high risk
print("Risky Instances (High Posterior Probability of Unsafe):")
print(risky_instances)

# Compute cumulative posterior probability for risky instances
# Extract the probabilities for the 'Unsafe' class
risky_instances_probabilities = test_data_with_proba['unsafe_proba']
